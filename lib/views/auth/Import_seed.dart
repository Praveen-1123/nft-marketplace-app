...
class ImportSeed extends StatefulWidget {
  const ImportSeed({Key key}) : super(key: key);
...
class _ImportSeedState extends State<ImportSeed> {
  TextEditingController _seed = new TextEditingController();
  TextEditingController _password = new TextEditingController();
  bool isLoading = false;

  void validateForm() async {
    int seedlength = ' '.allMatches(_seed.text.trim()).length;
    if (seedlength != 11) {
      GetXSnackBar()
          .errorMessage("Invalid Seed", "Please enter a valid seed pharse");
      setState(() {
        isLoading = false;
      });
    } else {
      ...
      if (validation == null) {
        ...
        bool valid = await AuthServices()
            .saveWithSeed(context, _seed.text, _password.text);
        if (!valid) {
          isLoading = false;
        }
      }
                ...
                children: [
                  ...
                  FadeInRight(
                    ...
                    child: Container(
                      ...
                      padding:
                          const EdgeInsets.only(left: 5, top: 30, bottom: 15),
                      child: Text(
